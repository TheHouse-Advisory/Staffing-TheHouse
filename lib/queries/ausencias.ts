...
export async function fetchAusenciasMes(
  ...
  const { data: personasRaw, error: pErr } = await supabase
    .from("persona")
    .select("id, nombre, apellido, cargo_actual, iniciales, fecha_ingreso, is_leverager, referente")
    .eq("activo", true)
    .order("apellido");
  ...
  interface PRow { id: string; nombre: string; apellido: string; cargo_actual: string | null; iniciales?: string | null; fecha_ingreso: string | null; is_leverager: boolean; referente: boolean }
